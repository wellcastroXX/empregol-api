...
import { CreateMediaDTO, UploadMediaDTO, UpdateMediaDTO } from './athlete-video.dto';
...
export class AthleteVideoService {
  ...
  /** Persists an uploaded video/photo file and creates its media record. */
  async addUpload(userId: string, file: Express.Multer.File, dto: UploadMediaDTO) {
    const athlete = await this.repo.findAthleteByUserId(userId);
    if (!athlete) throw new NotFoundError('Perfil de atleta não encontrado');
    ...
    return this.repo.create(athlete.id, {
      ...dto,
      mediaType,
  ...
  async update(userId: string, mediaId: string, dto: UpdateMediaDTO) {
    ...
    return this.repo.update(mediaId, dto);
  }
